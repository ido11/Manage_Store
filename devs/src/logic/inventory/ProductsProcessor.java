...
import presentation.inventory.printers.ProductsPrinter;
import presistence.Repository;
import presistence.dao.inventory.CategoryDAO;
import presistence.dao.inventory.DiscountsDAO;
import presistence.dao.inventory.ProductsDAO;
import presistence.dao.inventory.StockDAO;

import java.util.*;

public class ProductsProcessor implements InventoryProcessor {
    ...
    public ProductsProcessor(InventoryPrinter prompter, Repository repo) {
    ...
    private void processViewProductByCategory() {
        ...
        switch(((CategoryPrinter)prompter.getPrinter(Modules.CATEGORIES)).promptViewByCategory())
        {
            case 1:
                ...
                if(((CategoryDAO)repo.getInventoryDAO(Modules.CATEGORIES)).findCategoryByKey(catID) != null)
                {
                    products = ((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).findAllByCategory(null, catID);
            ...
            case 2:
                ...
                products = ((ProductsDAO) repo.getInventoryDAO(Modules.PRODUCTS)).findAllByCategory(name, 0);
                if(products.size() != 0) {
        ...
        switch(prompter.byBarcodeNamePrompt("Products", "Find a product to display by:"))
        {
            case 1:
                ...
                if (barcode == prompter.RETURN)
                    break;
                Product product = ((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).findByKey(new Product(barcode));
            ...
            case 2:
                ...
                if (name == null)
                    break;
                List<Product> products = ((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).findAllLike(name);
    ...
    private boolean processDeleteProduct() {
        ...
        if(delete != null && !cancel && delete) {
            ((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).delete(new Product(barcode));
            List<StockProducts> stock = ((StockDAO)repo.getInventoryDAO(Modules.STOCK)).findAllInventoryByBarcode(activeBranch, barcode);
            for(StockProducts s : stock)
                ((StockDAO)repo.getInventoryDAO(Modules.STOCK)).delete(s);
    ...
    private boolean processAddNewProduct() {
        ...
        Product p;
        if((p = ((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).findByKey(new Product(barcode))) != null) {
        ...
        if(productDetails.length == 1) {
            ...
            ((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).insert(new Product(
                    barcode, productDetails[0], productDetails[1], Double.parseDouble(productDetails[2]), Double.parseDouble(productDetails[3]),
                    Double.parseDouble(productDetails[3]), Double.parseDouble(productDetails[2]), Integer.parseInt(productDetails[4]),Double.parseDouble(productDetails[5])));
            ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).insert(new StockProducts(barcode, new Date(1, 1, 2100), -1, 0));
            ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).insert(new StockProducts(barcode, new Date(1, 1, 2100), -2, 0));

            if(!mainCategory.equals(""))
                ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).insert(new ProductCategory(barcode, mainCategory, 1));
            for(Map.Entry<Integer,String> subCat : subCategories)
                ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).insert(new ProductCategory(barcode, subCat.getValue(), subCat.getKey()));
        ...
        if(value != null) {
            switch (value.getKey()) {
                case 1:
                    ...
                    if (((ProductsDAO) repo.getInventoryDAO(Modules.PRODUCTS)).findByKey(new Product(newBarcode)) == null)
                ...
                case 7:
                    ...
                    String mainCategory = ((ProductsPrinter)prompter.getPrinter(Modules.PRODUCTS)).addProductMainCategory();
                    if (!mainCategory.equals("")) {
                        ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).deleteAllByProduct(new ProductCategory(p.getBarcode(), mainCategory, 1));
                        List<Map.Entry<Integer, String>> subCategories = new ArrayList<>();
                        processEditProductCategories(mainCategory, subCategories);
                        ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).insert(new ProductCategory(
                                ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).findByName(new ProductCategory(mainCategory)).getCategoryID(), p.getBarcode(), mainCategory, 1));
                        for (Map.Entry<Integer, String> subCat : subCategories)
                            ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).insert(new ProductCategory(
                                    ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).findByName(new ProductCategory(subCat.getValue())).getCategoryID(), p.getBarcode(), subCat.getValue(), subCat.getKey()));
            ...
            if (value.getKey() != 7) {
                ((ProductsDAO) repo.getInventoryDAO(Modules.PRODUCTS)).update(p, p.getBarcode(), 0, 0);
    ...
    private void processEditProductCategories(String mainCategory, List<Map.Entry<Integer,String>> subCategories) {
        if(!mainCategory.equals("")) {
            if((((CategoryDAO)repo.getInventoryDAO(Modules.CATEGORIES)).findByName(new ProductCategory(mainCategory))) == null) {
                ...
                if(!mainCategory.equals("")) {
                    ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).insertCategory(new ProductCategory(mainCategory));
            ...
            if (((ProductsPrinter)prompter.getPrinter(Modules.PRODUCTS)).checkAddProductsSubCategory()) {
                ...
                while (!subCategory.equals("q") && !subCategory.equals("Q")) {
                    if((((CategoryDAO)repo.getInventoryDAO(Modules.CATEGORIES)).findByName(new ProductCategory(subCategory))) == null) {
                        ...
                        if(!subCategory.equals("q") && !subCategory.equals("Q")) {
                            ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).insertCategory(new ProductCategory(subCategory));
    ...
    private Product processGetProductDetails(boolean wait, String message)
    {
        ...
        List<ProductCategory> categories = ((CategoryDAO)repo.getInventoryDAO(Modules.CATEGORIES)).findAllByProduct(p);
    ...
    private void reDiscount(Product p) {
        List<Discount> discounts = ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).findByBarcode(
    ...
    private void processPrintAllProducts() {
        ((ProductsPrinter)prompter.getPrinter(Modules.PRODUCTS)).printProducts(((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).findAll(), true, "Press ENTER to return to menu", false, true);
        prompter.printMessage(false, null);
    }
