...
import logic.inventory.models.Discount;
...
import presentation.inventory.printers.DiscountsPrinter;
...
public class DiscountProcessor implements InventoryProcessor {

    private Repository repo; //A repository instance used to communicate with database
    private InventoryPrinter prompter;

    public DiscountProcessor(InventoryPrinter prompter, Repository repo) {
        ...
        while (!menu) {
            ...
            switch(((DiscountsPrinter)prompter.getPrinter(Modules.DISCOUNTS)).printMenu(discounter))
            {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    processPrintAllDiscounts("Supplier");
                    break;
                case 6:
    ...
    private void processDisplayDiscountedCategories(String discounter) {
        List<ProductCategory> categories = ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).findAllDiscounted(discounter);
    ...
    private void processViewDiscounts(String discounter) {
        ...
        switch(prompter.byBarcodeNamePrompt("Discounts", "Find discount to display by:"))
        {
            case 1:
                ...
                if (barcode == prompter.RETURN)
                    break;
                List<Discount> discounts = ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).findByBarcode(new Discount(barcode, discounter), onlyActive, true);
            ...
            case 2:
                ...
                if (name == null)
                    break;
                List<Map.Entry<String, Discount>> discountsByName = ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).findAllLike(name,discounter, onlyActive);
        ...
        switch(((DiscountsPrinter)prompter.getPrinter(Modules.DISCOUNTS)).byProductCategoryDiscountPrompt("Product", "cancel discounted products")) {
            case 1:
                switch (prompter.byBarcodeNamePrompt("Discounts", "Find discount to remove by:")) {
                    case 1: // Cancel by barcode
                        while (!valid) {
                            ...
                            if (barcode == prompter.RETURN)
                                break;
                            List<Discount> discounts = ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).findByBarcode(new Discount(barcode, discounter), true, true);
                            if (discounts.size() == 0) //No discounts for current barcode
                                prompter.printMessage(true, "No matching active discounts for barcode number " + barcode);
                            else {
                    ...
                    case 2: //Cancel by name
                        ...
                        if (name == null)
                            break;
                        List<Map.Entry<String, Discount>> discounts = ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).findAllLike(name, discounter, true);
                ...
                if (discountID == prompter.RETURN)
                    cancel = true;
                if (!cancel) {
                    Discount d = ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).findByKey(new Discount(discountID, discounter));
                    if (discountID != -1) {
                        Date date;
                        ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).update(new Discount(d.getDiscountID(), d.getBarcode(),
            ...
            case 2:
                ...
                if(name != null) {
                    ...
                    if ((category = ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).findByName(new ProductCategory(name))) != null) {
                        ...
                        if(!isDiscounted)
                            ...
                            Date date = null;
                            List<Discount> d;
                            List<Product> products = ((ProductsDAO) repo.getInventoryDAO(Modules.PRODUCTS)).findAllByCategory(name, 0);
                            ...
                            for(Product p : products) {
                                d = ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).findByBarcode(new Discount(p.getBarcode()), true, false);
                                for(Discount dis : d) {
                                    ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).update(new Discount(
                            ...
                            ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).update(category, 0, category.getName(), 0);
    ...
    public void reDiscount(Discount discount) {
        ...
        Product p = ((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).findByKey(new Product(discount.getBarcode()));
    ...
    private boolean processAddNewDiscount(String discounter) { //TODO add by category
        ...
        switch(((DiscountsPrinter)prompter.getPrinter(Modules.DISCOUNTS)).byProductCategoryDiscountPrompt("Barcode", "discount products")) {
            ...
            int barcode = prompter.barcodePrompt();
            if (barcode != prompter.RETURN) {
                if (((ProductsDAO) repo.getInventoryDAO(Modules.PRODUCTS)).findByKey(new Product(barcode)) != null) {
                    List<Discount> discounts = ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).findByBarcode(new Discount(barcode, discounter), true, true);
                    ...
                    if (percentage == null) {
                        ...
                        ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).insert(new Discount(
    ...
    public void discountSupplier(int barcode, double percentage)
    {
        String discounter = "Supplier";
        if (((ProductsDAO) repo.getInventoryDAO(Modules.PRODUCTS)).findByKey(new Product(barcode)) != null) {
                String timeStamp = new SimpleDateFormat("yyyy-MM-dd").format(Calendar.getInstance().getTime());
                ((DiscountsDAO) repo.getInventoryDAO(Modules.DISCOUNTS)).insert(new Discount(
                        0, barcode, discounter, percentage, Date.parseDate(timeStamp), null));
    ...
    private void processPrintAllDiscounts(String discounter) {
        ((DiscountsPrinter)prompter.getPrinter(Modules.DISCOUNTS)).printDiscounts(((DiscountsDAO)repo.getInventoryDAO(Modules.DISCOUNTS)).findAllByDiscounter(discounter,
