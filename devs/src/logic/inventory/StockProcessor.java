...
import presistence.dao.inventory.LocationsDAO;
...
public class StockProcessor implements InventoryProcessor {
    ...
    public StockProcessor(InventoryPrinter prompter, Repository repo) {
    ...
    private void processMoveStock() {
        switch(prompter.byBarcodeNamePrompt("Branch Stock", "Choose how to find a product to move:")) {
            ...
            case 2:
                ...
                List<Product> prod = ((ProductsDAO)repo.getInventoryDAO(Modules.PRODUCTS)).findAllLike(name);
                if(prod.size() != 0) {
    ...
    private void processRemoveFromStock() {
        ...
        if(barcode != prompter.RETURN) {
            ...
            if ((p = (((ProductsDAO) repo.getInventoryDAO(Modules.PRODUCTS)).findByKey(new Product(barcode)))) != null) {
                List<StockProducts> availableToRemove = ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).findByBarcodeAndLocation(activeBranch, barcode);
                ...
                if (availableToRemove.size() != 0) {
                    ...
                    if (((StockPrinter) prompter.getPrinter(Modules.STOCK)).promptDeletion()) {
                        ...
                        for (StockProducts stock : availableToRemove) {
                            ...
                            stock.setQuantity(0);
                            ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).update(stock, stock.getBarcode(), stock.getExpiration_date(), stock.getLocationID());
                        }
        ...
        if (quantity != prompter.ILLEGAL) {
            ...
            if (date != null) {
                ...
                if (location != null) {
                    ...
                    ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).insert(new StockProducts(p.getBarcode(), date, actual.getLocationID(), quantity));
                    ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).insert(new StockProducts(p.getBarcode(), date, switched.getLocationID(), 0));
    ...
    private void moveStock(int barcode) {
        ...
        List<StockProducts> products = ((StockDAO)repo.getInventoryDAO(Modules.STOCK)).findAmountByLocations(activeBranch, barcode);
        if(products.size() > 0)
        {
            ...
            if(location != null) {
                ...
                if (stock != null) {
                    Location current = ((LocationsDAO) repo.getInventoryDAO(Modules.LOCATIONS)).findLocation(location);
                    ...
                    if (quantity <= stock.getQuantity()) {
                        ...
                        Location actual;
                        if ((actual = ((LocationsDAO) repo.getInventoryDAO(Modules.LOCATIONS)).findLocation(toMove)) == null) {
                            ((LocationsDAO) repo.getInventoryDAO(Modules.LOCATIONS)).insert(toMove);
                            actual = ((LocationsDAO) repo.getInventoryDAO(Modules.LOCATIONS)).findLocation(toMove);
                        }
                        stock.setLocationID(current.getLocationID());
                        stock.setQuantity(stock.getQuantity() - quantity);
                        if (stock.getQuantity() == 0)
                            ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).delete(stock);
                        else
                            ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).update(stock, stock.getBarcode(), stock.getExpiration_date(), actual.getLocationID());
                        stock.setQuantity(quantity);
                        stock.setLocationID(actual.getLocationID());
                        ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).insert(stock);
    ...
    private void processDisplayStockDetails() {
        ...
        if(barcode != prompter.RETURN) {
            Product p;
            if ((p = (((ProductsDAO) repo.getInventoryDAO(Modules.PRODUCTS)).findByKey(new Product(barcode)))) != null) {
                List<StockProducts> available_locations = ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).findByBarcodeAndLocation(activeBranch, barcode);
                List<StockProducts> available = ((StockDAO) repo.getInventoryDAO(Modules.STOCK)).findAllInventoryByBarcode(activeBranch, barcode);
    ...
    private void processDisplayAllStock() {
        ...
        switch(choice)
        {
            case 0:
                prompter.printMessage(false, null);
                List<StockProducts> inventory = ((StockDAO)repo.getInventoryDAO(Modules.STOCK)).findAllByLocation(activeBranch);
                ((StockPrinter)prompter.getPrinter(Modules.STOCK)).printAllStockByLocation(inventory, true);
                prompter.printMessage(false, null);
                break;
            case 1:
                prompter.printMessage(false, null);
                inventory = ((StockDAO)repo.getInventoryDAO(Modules.STOCK)).findAllInventory(activeBranch, null);
    ...
    private void processDisplayAllStockByCategory() {
        ...
        ProductCategory p;
        if ((p = ((CategoryDAO) repo.getInventoryDAO(Modules.CATEGORIES)).findCategoryByKey(catID)) != null) {
            List<StockProducts> cat_stock = ((StockDAO)repo.getInventoryDAO(Modules.STOCK)).findAllInventory(activeBranch, p);
            if(cat_stock.size() != 0) {
