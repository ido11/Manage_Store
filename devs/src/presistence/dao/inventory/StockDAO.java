...
public class StockDAO implements DAO<StockProducts, Integer, Date, Integer> {
    ...
    public Result updateAll(StockProducts stock, Integer key1, Date key2, Integer key3) {
        String sql = "UPDATE stock SET quantity = ?, expiration_date = ?, locationID = ?"+
                    "WHERE barcode = ? AND expiration_date = ? AND locationID = ? ";
        try {
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, stock.getQuantity());
            ...
            pstmt.setInt(4, key1);
            pstmt.setString(5, key2.toString());
            pstmt.setInt(6, key3);
            pstmt.executeUpdate();
            conn.commit();
            return Result.SUCCESS;
    ...
    public StockProducts findByKey(StockProducts stockProducts) {
        String sql = "SELECT * FROM stock WHERE barcode = ? AND expiration_date = ? AND locationID = ?";
        try {
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, stockProducts.getBarcode());
            pstmt.setString(2, stockProducts.getExpiration_date().toString());
            pstmt.setInt(3, stockProducts.getLocationID());
            ResultSet rs = pstmt.executeQuery();
            if(rs.next())
                return new StockProducts (rs.getInt("barcode"),
    ...
    @Override
    public LinkedList<StockProducts> findByVal(Object id) {
        return null;
    }
}
